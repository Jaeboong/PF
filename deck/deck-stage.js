...
(() => {
  ...
  const stylesheet = `
    ...
    :host([data-scroll]) .stage { position: relative; inset: auto; display: block; overflow: hidden; }
    :host([data-scroll]) .canvas { transform-origin: top left; box-shadow: none; }
    :host([data-scroll]) ::slotted(*) {
    ...
    :host([data-scroll]) ::slotted(*:last-child) { margin-bottom: 0; }
    :host([data-scroll]) .overlay,
    :host([data-scroll]) .rail,
    :host([data-scroll]) .rail-resize,
    :host([data-scroll]) .ctxmenu,
    :host([data-scroll]) .confirm-backdrop { display: none !important; }
  ...
  class DeckStage extends HTMLElement {
    ...
    _fit() {
      ...
      if (this.hasAttribute('data-scroll')) {
        const scale = window.innerWidth / this.designWidth;
        const visibleSlides = this._slides.filter((slide) => !slide.hasAttribute('data-deck-skip')).length;
        const scrollHeight = (this.designHeight * visibleSlides) + (Math.max(visibleSlides - 1, 0) * 16);
        this._canvas.style.height = scrollHeight + 'px';
        this._canvas.style.transform = `scale(${scale})`;
        if (stage) {
          stage.style.left = '0';
          stage.style.height = (scrollHeight * scale) + 'px';
        }
        try {
          window.parent.postMessage({ type: 'pf-scroll-height', height: scrollHeight * scale }, location.origin);
        } catch (e) {}
        return;
      }
